...
- Localized assistant identity to assist policyholders 'in Sri Lanka' with Sri Lankan context
- Updated monetary references to use 'Rs.' or 'LKR' formatting instead of dollar signs throughout the system
- Implemented proper currency formatting with .toLocaleString() for thousands separators in all monetary displays
- Enhanced documentation to reflect current assistant identity and Sri Lankan localization
...
## Introduction
This document explains the AI-powered Claim Assistant Service that provides intelligent, context-aware assistance for vehicle insurance claims in Sri Lanka. The service features the **Flash Claim Assistant**, a helpful and knowledgeable AI that assists policyholders with their vehicle insurance claims specifically tailored for the Sri Lankan market. It covers how the chatbot answers claim-related queries, provides status updates and guidance, maintains conversation history, and integrates with claim data, policy information, and user history to deliver personalized responses with Sri Lankan Rupees (Rs./LKR) formatting. It also includes examples of common query patterns, response templates, escalation procedures, and guidance on customizing assistant behavior and adding domain-specific knowledge.

## Project Structure
The Claim Assistant Service is implemented as a backend service integrated into the Express routes and persisted via Prisma. The frontend exposes an interactive chat interface within the claim detail page with Sri Lankan Rupees formatting.
...
- Context-aware response generation: Builds a rich claim context from related entities (vehicle, policy, damage assessment, repair estimate, payout, documents) and injects it into the model prompt with Sri Lankan Rupees formatting.
...
- Sri Lankan localization: All monetary values are displayed in Sri Lankan Rupees (Rs./LKR) with proper thousands separator formatting using .toLocaleString().
...
- Service loads claim context and recent chat history, constructs a system prompt plus claim context with Sri Lankan Rupees formatting, and calls Gemini.
...
- Constructs a structured context string including:
  ...
  - Policy provider, coverage type, deductible in Rs. format
  - Damage assessment summary and individual damages
  - Repair estimate totals in Rs. format with thousands separators
  - Insurance payout estimate in Rs. format with thousands separators
  - Required document statuses (LICENSE, REGISTRATION, ACCIDENT_REPORT)
- Initializes a chat session with a system prompt defining assistant responsibilities and Sri Lankan context, followed by claim context, then appends prior conversation history.
- Persists both user and assistant messages to enable persistent multi-turn conversations per claim.

**Updated** Enhanced with Sri Lankan Rupees formatting and localized context for Sri Lankan policyholders.

```mermaid
flowchart TD
Start(["Receive chat message"]) --> LoadClaim["Load claim + relations + last 20 messages"]
LoadClaim --> BuildContext["Build context from claim data with Rs. formatting"]
BuildContext --> InitChat["Initialize chat with system prompt + Sri Lankan claim context"]
...
- InsurancePolicy: provider, coverage type, deductible in Sri Lankan Rupees used for payout explanations.
- DamageAssessment: overall severity and drivability assessment.
- RepairEstimate: itemized costs in Rs. format and total cost/time.
- InsurancePayout: estimated payout in Rs. format after deductible.
...
- Shows monetary values in Sri Lankan Rupees (Rs.) format with thousands separators.
...
**Updated** Enhanced with Sri Lankan Rupees formatting throughout the interface.
...
- "Explain the estimate": Breaks down parts vs labor in Rs. format, mentions deductible impact, and estimated repair time.
...
These are generated dynamically using the constructed context and Sri Lankan-localized system prompt.
...
- System prompt customization: Modify the system prompt to adjust tone, scope, and Sri Lankan context.
- Model selection: Change the Gemini model name in the utility to balance speed vs quality.
- Knowledge bases: Extend the context-building logic to include additional data sources (e.g., Sri Lankan policy rules, local repair shop catalogs) to inform responses.
...
- Frontend React components for user interaction with Sri Lankan Rupees formatting.
...
## Conclusion
The Claim Assistant Service delivers personalized, context-aware support throughout the claims lifecycle through the **Flash Claim Assistant**, specifically designed for Sri Lankan policyholders. By combining rich claim data, policy details, and conversation history with a configurable AI model, it provides clear guidance, status updates, and actionable recommendations with Sri Lankan Rupees (Rs./LKR) formatting. The consistent branding and localization ensure users receive responses from a unified assistant identity that understands the Sri Lankan insurance context. Extensibility points allow customization of assistant behavior, addition of domain-specific knowledge, and adaptation to different claim types.
...
SVC->>LLM : Generate explanation with Sri Lankan context
LLM-->>SVC : Explanation text in Rs. format
...
- Extend context building in the assistant service to include additional data sources (e.g., Sri Lankan policy rules, local vendor catalogs).
- Update the system prompt to instruct the model to use the new knowledge base appropriately with Sri Lankan context.
- Test with representative queries to ensure accurate and safe responses.

[No sources needed since this section provides conceptual guidance]

### System Prompt Configuration
The Flash Claim Assistant uses a comprehensive system prompt that defines its identity and Sri Lankan responsibilities:

**Current System Prompt:**
```
You are the Flash Claim Assistant, a helpful and knowledgeable AI that assists policyholders with their vehicle insurance claims in Sri Lanka.
