...
import { normalizeGarageItems, estimateTotals, DEFAULT_LABOR_RATE, type EstimatePartRow } from '../../utils/garageEstimate';
...
export function GarageClaimDetailPage() {
  ...
  const fetchClaim = () =>
    garageApi.get(`/claims/${id}`).then((r) => {
      setClaim(r.data);
      // Pre-populate from the existing garage estimate or the AI estimate
      const existing = r.data.garageEstimate;
      const ai = r.data.repairEstimate;
      const source = normalizeGarageItems(existing ? existing.items : (ai ? ai.items : []));
      setParts(source.parts);
      setLaborHours(source.laborHours);
  ...
  const updatePartCost = (idx: number, value: string) => {
    setParts((prev) => {
      const updated = [...prev];
      updated[idx] = { ...updated[idx], partCost: parseFloat(value) || 0 };
      return updated;
    });
  };

  const addPart = () => {
    setParts((prev) => [...prev, { damageType: 'other', partName: '', partCost: 0, addedByGarage: true }]);
    setEditMode(true);
  };
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-4">
          ...
          <div className="bg-purple-50 rounded-lg p-2 text-center"><p className="text-xs text-purple-600">Labor</p><p className="font-bold text-purple-900">Rs. {totals.laborCost.toLocaleString()}</p></div>
          <div className="bg-sky-50 rounded-lg p-2 text-center"><p className="text-xs text-sky-600">Paint</p><p className="font-bold text-sky-900">Rs. {paintMaterials.toLocaleString()}</p></div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 text-left text-gray-500 text-xs uppercase">
                ...
                <th className="pb-2 text-right">Part Cost (Rs.)</th>
                {editMode && <th className="pb-2" />}
              </tr>
            </thead>
            <tbody>
              {parts.map((part, idx) => (
                <tr key={idx} className={`border-b border-gray-100 ${part.addedByGarage ? 'bg-orange-50' : ''}`}>
                  <td className="py-2">
                    {editMode ? (
                      <select value={part.damageType} onChange={(e) => updatePartText(idx, 'damageType', e.target.value)}
                    ...
                    ) : <span className="capitalize">{part.damageType.replace(/_/g, ' ')}</span>}
                  </td>
                  <td className="py-2">
                    {editMode ? (
                      <input value={part.partName} onChange={(e) => updatePartText(idx, 'partName', e.target.value)}
                        className="px-2 py-1 border border-gray-300 rounded text-xs w-full" />
                    ) : part.partName}
                  </td>
                  <td className="py-2 text-right">
                    {editMode ? (
                      <div className="flex justify-end">
                        <input type="number" value={part.partCost} onChange={(e) => updatePartCost(idx, e.target.value)}
                          onFocus={(e) => e.target.select()}
                          className="px-2 py-1 border border-gray-300 rounded text-xs w-24" />
                      </div>
                    ) : `Rs. ${part.partCost.toLocaleString()}`}
                  </td>
                  {editMode && (
                    <td className="py-2">
                      <button onClick={() => removePart(idx)} className="p-1 text-red-500 hover:text-red-700">
                ...
                <td className="py-2">
                  {editMode ? (
                    <div className="flex items-center gap-1.5 text-xs">
                      <input type="number" step="0.5" value={laborHours} onChange={(e) => setLaborHours(parseFloat(e.target.value) || 0)}
                        onFocus={(e) => e.target.select()}
                        className="px-2 py-1 border border-gray-300 rounded text-xs w-16" />
                      <span className="text-gray-500">hours @ Rs.</span>
                    ...
                    <span className="text-gray-600">{laborHours}h @ Rs. {laborRate.toLocaleString()}/h</span>
                  )}
                </td>
                <td className="py-2 text-right font-medium">Rs. {totals.laborCost.toLocaleString()}</td>
                {editMode && <td className="py-2" />}
              </tr>

              {/* Paint & materials — single line */}
              <tr className="border-b border-gray-100 bg-sky-50">
                <td className="py-2 font-medium text-sky-900">Paint & Materials</td>
                <td className="py-2 text-xs text-gray-500">Paint, filler and other consumables</td>
                <td className="py-2 text-right">
                  {editMode ? (
                    <div className="flex justify-end">
                      <input type="number" value={paintMaterials} onChange={(e) => setPaintMaterials(parseFloat(e.target.value) || 0)}
                        onFocus={(e) => e.target.select()}
                        className="px-2 py-1 border border-gray-300 rounded text-xs w-24" />
                    </div>
                  ) : `Rs. ${paintMaterials.toLocaleString()}`}
                </td>
                {editMode && <td className="py-2" />}
              </tr>
              ...
              <tr className="font-semibold">
                ...
                <td className="py-2 text-right">Rs. {totals.totalCost.toLocaleString()}</td>
                {editMode && <td className="py-2" />}
              </tr>
            </tbody>
          </table>
